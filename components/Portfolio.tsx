...
type Category = "Rekonstrukce" | "Koupelny";
...
const projectData: {
  ...
  { category: "Rekonstrukce", images: [1, 2, 3, 4].map((i) => `/photos/praha-12-a/${i}.jpeg`), imageAlt: "Praha 12" },
  { category: "Rekonstrukce", images: [1, 2, 3, 4].map((i) => `/photos/praha-12-b/${i}.jpeg`), imageAlt: "Praha 12" },
  { category: "Rekonstrukce", images: [1, 2, 4].map((i) => `/photos/praha-12-c/${i}.jpeg`), imageAlt: "Praha 12" },
  { category: "Rekonstrukce", images: [1, 2, 4, 6].map((i) => `/photos/praha-4/${i}.jpeg`), imageAlt: "Praha 4" },
  { category: "Rekonstrukce", images: [1, 2, 3, 4].map((i) => `/photos/praha-8-strizkov/${i}.jpeg`), imageAlt: "Praha 8 Střížkov" },
  { category: "Rekonstrukce", images: [1, 3, 6, 9].map((i) => `/photos/praha-stodulky-a/${i}.jpeg`), imageAlt: "Praha Stodůlky" },
  { category: "Rekonstrukce", images: [1, 3, 6, 8].map((i) => `/photos/praha-stodulky-b/${i}.jpeg`), imageAlt: "Praha Stodůlky" },
  { category: "Rekonstrukce", images: [1, 2, 4].map((i) => `/photos/mesice-u-prahy/${i}.jpeg`), imageAlt: "Měšice u Prahy" },
  { category: "Koupelny", images: [1, 2, 3].map((i) => `/photos/mesice-koupelna/${i}.jpeg`), imageAlt: "Měšice u Prahy" },
  { category: "Koupelny", images: [1, 2, 4].map((i) => `/photos/strizkov-koupelna/${i}.jpeg`), imageAlt: "Praha 8 Střížkov" },
  { category: "Koupelny", images: [1, 2, 3].map((i) => `/photos/stodulky-koupelna/${i}.jpeg`), imageAlt: "Praha Stodůlky" },
  { category: "Rekonstrukce", images: [1, 3, 5, 7].map((i) => `/photos/chomutov/${i}.jpeg`), imageAlt: "Chomutov" },
];

const filterKeys = ["all", "Rekonstrukce", "Koupelny"] as const;
  ...
  return (
    <article className="group relative cursor-pointer overflow-hidden">
      ...
      <div className="absolute inset-0 flex items-end bg-foreground/55 p-6 opacity-0 transition-opacity duration-500 md:group-hover:opacity-100">
        <div>
          <h3 className="mb-1 text-xl text-background">{title}</h3>
      ...
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-foreground/75 to-transparent p-5 md:hidden">
        <h3 className="text-base text-background">{title}</h3>
        <p className="label-eyebrow text-[0.56rem] text-background/55">{location}</p>
      </div>
    </article>
  );
}
...
export default function Portfolio() {
  ...
  const activeKey = filterKeys[activeIdx];

  const visible = activeKey === "all"
    ? projectData
    : projectData.filter((proj) => proj.category === activeKey);
      ...
      <div className="container-max section-px">
        ...
        <div className="grid grid-cols-1 gap-2 md:grid-cols-2 md:gap-2 lg:grid-cols-3">
          {visible.map((proj, idx) => {
            const txProj = p.projects[projectData.indexOf(proj)];
            return (
              <ProjectCard
                key={idx}
                images={proj.images}
                title={txProj?.title ?? proj.imageAlt}
                location={proj.imageAlt}
                imageAlt={proj.imageAlt}
                prevLabel={p.prevPhoto}
                nextLabel={p.nextPhoto}
                photoNumLabel={p.photoNum}
              />
            );
          })}
        </div>
